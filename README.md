...
Para atender ao requisito da disciplina, foi utilizada o Sequelize - uma biblioteca ORM - para manipular o banco de dados de maneira simples, organizada e sem escrever SQL manualmente. Isso garante mais segurança, melhor manutenção e um padrão profissional de desenvolvimento.

✔️ Autenticação
A API implementa autenticação baseada em tokens - JWT -, garantindo que apenas usuários autorizados possam realizar operações como cadastro, edição e exclusão de filmes. Endpoints sensíveis estão protegidos, seguindo as boas práticas de segurança em APIs modernas.
...
Se atente ao usuário cadastrado no banco de dados. No caso, utilizamos Postgre, altere os valores de acordo com seus dados.
